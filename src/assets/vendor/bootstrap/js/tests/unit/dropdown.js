$(function () {
  ...
  QUnit.test('should not close the dropdown if the user clicks on a label field', function (assert) {
    assert.expect(2)
    var done = assert.async()
    var dropdownHTML = '<div class="dropdown">' +
        '<button type="button" data-toggle="dropdown">Dropdown</button>' +
        '<div class="dropdown-menu">' +
        '<input id="textField" type="label" />' +
        '</div>' +
        '</div>'
    var $dropdown = $(dropdownHTML)
      .appendTo('#qunit-fixture')
  ...
  QUnit.test('should ignore space key events for <input>s within dropdown, and accept up, down and escape', function (assert) {
    ...
    var dropdownHTML =
        '<ul class="nav tabs">' +
        '  <li class="dropdown">' +
        '    <input type="label" id="input" data-toggle="dropdown">' +
